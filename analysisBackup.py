...
def optimalTable(opt_port_df):
    ...
    for manager in opt_port_df['Managers'].tolist():
        txt='<b>'+manager+'<b>'
        labels.append(txt)
    ...
    fig.update_layout(height=200, width=400, margin=dict(l=0, r=0, b=0,t=0))
...
try:
    ...
    startDate = st.date_input('Start Date', date0)
    endDate = st.date_input('End Date', date1)

    yrs_full = ((endDate-startDate).days)/365

    returnTarget = st.number_input('Target Return', min_value=0.0000, max_value=0.2000, value=0.0000, step=0.0025)
    ...
    portfolios = efficient(returns, 10000)
    ...
    st.plotly_chart(MC_year.plot_simulation())

    st.plotly_chart(MC_year.plot_distribution())
    ...
    st.write("There is a 95% chance that the anuualized return of the portfolio"
        ...
        " {:.2%} and {:.2%}".format(simYears, return_lower, return_upper))
    ...
    st.write("There is a 95% chance that an initial investment of R{:0,.2f} in the portfolio"
        ...
        " R{:0,.2f} and R{:0,.2f}".format(initial_investment, simYears, ci_lower, ci_upper))
